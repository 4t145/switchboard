use crate::{
    resolve,
    resource::tls::file::{FileTlsCertResource, FileTlsCertResourceError},
};
use std::{
...
pub struct FileTls {
    pub name: String,
    #[serde(flatten)]
    pub resource: FileTlsResources,
    #[serde(default)]
    pub options: Option<switchboard_model::TlsOptions>,
}
...
#[serde(untagged)]
pub enum FileTlsResources {
    Single(FileTlsCertResource),
    Sni { sni: Vec<FileTlsSniItem> },
}
#[derive(
    Clone, Debug, Serialize, Deserialize, Hash, bincode::Encode, bincode::Decode, PartialEq, Eq,
)]
pub struct FileTlsSniItem {
    pub domain: String,
    #[serde(flatten)]
    pub resource: FileTlsCertResource,
}
...
pub enum ResolveConfigFileError {
    ...
    ResolveTlsCert {
        #[source]
        source: FileTlsCertResourceError,
        name: String,
    },
}
...
pub async fn fetch_config(
...
) -> Result<switchboard_model::Config, ResolveConfigFileError> {
    ...
    for file_tls in config.tls.into_iter() {
        ...
        let resolver = match file_tls.resource {
            FileTlsResources::Single(file_tls_cert_resource) => {
                let params = file_tls_cert_resource.fetch().await.map_err(|e| {
                    ResolveConfigFileError::ResolveTlsCert {
                        source: e,
                        name: name.clone(),
                    }
                })?;
            ...
            FileTlsResources::Sni { sni } => {
                let mut sni_map = BTreeMap::new();
                for item in sni.into_iter() {
                    let domain = item.domain;
                    let params = item.resource.fetch().await.map_err(|e| {
                        ResolveConfigFileError::ResolveTlsCert {
                            source: e,
                            name: name.clone(),
                        }
                    })?;
                    sni_map.insert(domain, params);
                }
                TlsResolver::Sni(sni_map)
            }
        };
        tls.insert(
            name,
    ...
    for service_config in config.tcp_services.into_iter() {
        ...
        let resolved_config = if let Some(link) = &service_config.config {
            let resolved = resolver.fetch(link).await.map_err(|source| {
        ...
        let resolved_service = switchboard_model::TcpServiceConfig {
        ...
        for bind in service_config.binds {
            ...
            tcp_routes.insert(
                bind.bind,
                switchboard_model::tcp_route::TcpRoute {
                    bind: bind.bind,
    ...
    match provider {
        "http" => {
                ...
                )
            {
                ...
                let resolved_instance_data = switchboard_model::services::http::InstanceData {
            ...
            let resolved_config = switchboard_model::services::http::Config {
