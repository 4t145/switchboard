...
pub struct HostnameTree<T> {
    ...
    pub(crate) host_exact: BTreeMap<String, T>,
    pub(crate) host_with_wildcard: HostnameMatcherNode<T>,
...
impl<T> Default for HostnameTree<T> {
    fn default() -> Self {
        Self {
            ipv4: BTreeMap::new(),
            ipv6: BTreeMap::new(),
            host_exact: BTreeMap::new(),
            host_with_wildcard: HostnameMatcherNode::new(),
            fallback: None,
        }
    }
}
...
impl<T> HostnameTree<T> {
    ...
    pub fn is_empty(&self) -> bool {
        self.ipv4.is_empty()
            && self.ipv6.is_empty()
            && self.host_exact.is_empty()
            && self.host_with_wildcard.is_empty()
    ...
    pub fn iter(&self) -> HostnameTreeIter<'_, T> {
        HostnameTreeIter {
            ...
            host_exact: self.host_exact.values(),
            host_with_wildcard: self.host_with_wildcard.iter(),
    ...
    pub fn iter_mut(&mut self) -> HostnameTreeIterMut<'_, T> {
        HostnameTreeIterMut {
            ...
            host_exact: self.host_exact.values_mut(),
            host_with_wildcard: self.host_with_wildcard.iter_mut(),
    ...
    pub fn get(&self, host: &str) -> Option<&T> {
        ...
        let data = if host.starts_with('[') {
            ...
            if let Ok(ipv4) = host.parse::<Ipv4Addr>() {
                self.ipv4.get(&ipv4)
            } else if let Some(data) = self.host_exact.get(host.to_ascii_lowercase().as_str()) {
                Some(data)
            } else {
                self.host_with_wildcard.get(host)
            }
        };
    ...
    pub fn get_mut(&mut self, host: &str) -> Option<&mut T> {
        ...
        let data = if host.starts_with('[') {
            ...
            } else if let Some(data) = self.host_exact.get_mut(host.to_ascii_lowercase().as_str()) {
                Some(data)
            } else {
                self.host_with_wildcard.get_mut(host)
            }
        };
    ...
    pub fn set(&mut self, host: &str, data: T) {
        ...
        if host.starts_with('[') {
            if let Some(ipv6) = host
                ...
                && let Ok(ipv6) = ipv6.parse::<Ipv6Addr>()
            {
            ...
            self.host_exact.insert(host.to_ascii_lowercase(), data);
        } else {
...
pub struct HostnameTreeIter<'a, T> {
    ...
    host_exact: std::collections::btree_map::Values<'a, String, T>,
    host_with_wildcard: HostnameMatcherNodeIter<'a, T>,
...
impl<'a, T: 'a> Iterator for HostnameTreeIter<'a, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if let Some(data) = self.host_exact.next() {
            return Some(data);
        }
        if let Some(data) = self.host_with_wildcard.next() {
            return Some(data);
        }
        self.fallback.next()
    }
...
pub struct HostnameTreeIterMut<'a, T> {
    ...
    host_exact: std::collections::btree_map::ValuesMut<'a, String, T>,
    host_with_wildcard: HostnameMatcherNodeIterMut<'a, T>,
...
#[derive(Debug)]
pub struct HostnameMatcherNodeIntoIter<T> {
    data: std::option::IntoIter<T>,
    children: std::collections::btree_map::IntoIter<String, HostnameMatcherNode<T>>,
    else_node: Option<Box<HostnameMatcherNodeIntoIter<T>>>,
}

impl<T> Iterator for HostnameMatcherNodeIntoIter<T> {
    type Item = T;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if let Some((_, node)) = self.children.next() {
            return node.into_iter().next();
        }
...
impl<T> IntoIterator for HostnameMatcherNode<T> {
    type Item = T;
    type IntoIter = HostnameMatcherNodeIntoIter<T>;

    fn into_iter(self) -> Self::IntoIter {
        HostnameMatcherNodeIntoIter {
            data: self.data.into_iter(),
            children: self.children.into_iter(),
            else_node: self.else_node.map(|node| Box::new(node.into_iter())),
        }
    }
}
#[derive(Debug)]
...
impl<T> HostnameMatcherNode<T> {
    pub fn is_empty(&self) -> bool {
        self.data.is_none() && self.children.is_empty() && self.else_node.is_none()
    }
    pub fn new() -> Self {
        Self::default()
    }
    ...
    pub fn iter_mut(&mut self) -> HostnameMatcherNodeIterMut<'_, T> {
        HostnameMatcherNodeIterMut {
            ...
            else_node: self
                .else_node
                .as_mut()
                .map(|node| Box::new(node.iter_mut())),
        }
    }
}
...
mod test {
    ...
    macro_rules! test_cases {
        ($tree: ident
            ...
            $($tree.set($rule, $rule);)*
            println!("{:#?}", $tree.host_with_wildcard);
            $(
                $(
