<script lang="ts">
	import {
		...
		isSniTlsResolver,
		type FileStyleTls,
		type FileStyleTlsResolver,
		type SniFileStyleTlsResolver
	} from '$lib/api/types';
	...
	type TlsCertParams = {
		certs: string; // LinkOrValue<PemsFile>
		key: string; // LinkOrValue<PemFile>
		ocsp: unknown | null;
	};
	export type Props = {
	...
	let singleModeCache = $state<TlsCertParams>(
		isSingleTlsResolver(value)
			? value
			: {
					certs: '',
					key: '',
	...
	let sniModeCache = $state<SniFileStyleTlsResolver>(
		isSniTlsResolver(value)
			? value
			: {
					sni: []
				}
...
{#snippet privateKeyEditor(value: string, onValueChange: (value: string) => void)}
	<PemInput {value} {onValueChange} required={true} label={'Private Key Content (PEM)'} />
{/snippet}
{#snippet certChainEditor(value: string, onValueChange: (value: string) => void)}
	<PemInput {value} {onValueChange} required={true} label={'Certificate Chain Content (PEM)'} />
{/snippet}
